...
/**
 * SEO AUDITOR #2: ON-PAGE SEO AUDIT
 * Analyzes titles, meta descriptions, H1-H3, URLs, internal linking, schema markup
 */
...
function addIssue(category, page, issue, severity = 'WARNING') {
  issueCount++;
  issues.push({
    id: issueCount,
    category,
...
function extractMetadata(content) {
  ...
  // Extract keywords
  const keyMatch = content.match(/keywords:\s*['"`]([^'"`]+)['"`]/);
  if (keyMatch) {
    metadata.keywords = keyMatch[1];
  }
  ...
  // Extract H1 tags (looking for h1 className in JSX)
  const h1Match = content.match(/<h1[^>]*className[^>]*>([^<]+)<\/h1>/);
  if (h1Match) {
    metadata.h1 = h1Match[1].trim();
  }

  // Extract H2 tags
  const h2Matches = content.matchAll(/<h2[^>]*className[^>]*>([^<]+)<\/h2>/g);
  for (const match of h2Matches) {
    metadata.h2s.push(match[1].trim());
  }

  // Extract H3 tags
  const h3Matches = content.matchAll(/<h3[^>]*className[^>]*>([^<]+)<\/h3>/g);
  for (const match of h3Matches) {
    metadata.h3s.push(match[1].trim());
  }
...
function auditTitleTag(page, title) {
  const issues_local = [];
  ...
  if (len < 30) {
    issues_local.push({
      issue: `Title too short: ${len} chars (ideal 50-60). Current: "${title}"`,
      severity: 'WARNING',
    });
  }
  if (len > 70) {
    issues_local.push({
      issue: `Title too long: ${len} chars (ideal 50-60). Current: "${title}"`,
      severity: 'WARNING',
    });
  }

  // Check for brand name at end (best practice)
  if (!title.includes('Wonderloud') && !title.includes('Rainuka')) {
    issues_local.push({
      issue: `Title missing brand identifier: "${title}"`,
      severity: 'MINOR',
    });
  }
  ...
  if (!title.match(/\|/)) {
    issues_local.push({
      issue: `Title lacks separator (pipe "|") for clarity: "${title}"`,
      severity: 'MINOR',
    });
  }
...
function auditMetaDescription(page, description) {
  const issues_local = [];
  ...
  if (len < 120) {
    issues_local.push({
      issue: `Meta description too short: ${len} chars (ideal 150-160). Current: "${description.substring(0, 80)}..."`,
      severity: 'WARNING',
    });
  }
  if (len > 170) {
    issues_local.push({
      issue: `Meta description too long: ${len} chars (ideal 150-160). Current: "${description.substring(0, 100)}..."`,
      severity: 'WARNING',
    });
  }

  // Check for CTA
  if (!description.match(/\b(learn|get|discover|find|explore|start|book|schedule|contact)\b/i)) {
    issues_local.push({
      issue: `Meta description missing clear CTA: "${description.substring(0, 80)}..."`,
      severity: 'MINOR',
    });
  }
...
function auditH1Tag(page, h1, title) {
  const issues_local = [];

  if (!h1) {
    issues_local.push({
      issue: `Missing H1 tag on page`,
      severity: 'CRITICAL',
    });
    return issues_local;
  }

  // H1 should not match title exactly (they have different audiences)
  if (h1.toLowerCase() === title?.toLowerCase()) {
    issues_local.push({
      issue: `H1 identical to title tag. H1 should be descriptive for users, title for search engines: "${h1}"`,
      severity: 'WARNING',
    });
  }
  ...
  if (h1.length > 80) {
    issues_local.push({
      issue: `H1 too long (${h1.length} chars, ideal 30-60): "${h1}"`,
      severity: 'MINOR',
    });
  }
...
function auditHeaderHierarchy(page, h1, h2s, h3s) {
  const issues_local = [];

  if (!h1) {
    issues_local.push({
      issue: `Missing H1: cannot establish header hierarchy`,
      severity: 'CRITICAL',
    });
    return issues_local;
  }
  ...
  if (h2s.length === 0 && h3s.length > 0) {
    issues_local.push({
      issue: `Hierarchy skip detected: H1 exists but no H2s before H3s (${h3s.length} H3s found)`,
      severity: 'WARNING',
    });
  }
  ...
  if (h2s.length > 8) {
    issues_local.push({
      issue: `Excessive H2s: ${h2s.length} H2s on page (ideal 3-5)`,
      severity: 'MINOR',
    });
  }

  // Check for keyword alignment
  const allHeaders = [h1, ...h2s, ...h3s];
  const hasKeywordInHeaders = allHeaders.some(
    (h) => h && h.match(/\b(therapy|therapist|mental health|perinatal|ADHD|career)\b/i)
  );

  if (!hasKeywordInHeaders) {
    issues_local.push({
      issue: `Headers lack primary keywords. Headers: ${allHeaders.join(' | ')}`,
      severity: 'MINOR',
    });
  }
...
function auditURLStructure(pathname) {
  const issues_local = [];

  if (pathname.includes('page.tsx')) {
    const url = pathname.replace(/app\/|\/page\.tsx/g, '').replace(/\\/g, '/');

    // Check for parameters
    if (url.includes('?') || url.includes('[')) {
      issues_local.push({
        issue: `URL uses parameters or dynamic segments: /${url}`,
        severity: 'MINOR',
      });
    }

    // Check readability
    if (url.match(/[a-z0-9]{15,}/)) {
      issues_local.push({
        issue: `URL slug unclear or too long: /${url}`,
        severity: 'MINOR',
      });
    }

    // Check for hyphens (best practice)
    if (url.includes('_')) {
      issues_local.push({
        issue: `URL uses underscores instead of hyphens: /${url}`,
        severity: 'MINOR',
      });
    }
  }
...
function auditInternalLinking(content, page) {
  const issues_local = [];
  ...
  for (const match of linkMatches) {
    ...
    // Check for weak anchor text
    if (text.match(/^(click here|read more|learn more|link|here)$/i)) {
      issues_local.push({
        issue: `Weak anchor text on ${page}: "${text}" → ${href}`,
        severity: 'MINOR',
      });
    }
  }
  ...
  if (linkCount === 0) {
    issues_local.push({
      issue: `No internal links found on ${page}`,
      severity: 'WARNING',
    });
  }
  ...
  if (linkCount < 2) {
    issues_local.push({
      issue: `Very few internal links (${linkCount}, ideal 2-4): ${page}`,
      severity: 'MINOR',
    });
  }
...
function auditSchemaMarkup(content, page) {
  const issues_local = [];

  const hasLocalBusiness = content.includes('LocalBusiness');
  const hasFAQSchema = content.includes('FAQPage');
  const hasAggregateRating = content.includes('aggregateRating') || content.includes('AggregateRating');
  const hasSchema = content.includes('application/ld+json');

  if (!hasSchema) {
    issues_local.push({
      issue: `No structured data (JSON-LD) found on ${page}`,
      severity: 'WARNING',
    });
  }

  // Check for missing LocalBusiness on key pages
  if ((page.includes('home') || page.includes('about')) && !hasLocalBusiness) {
    issues_local.push({
      issue: `LocalBusiness schema missing on ${page} (recommended for local service pages)`,
      severity: 'MINOR',
    });
  }

  // Check for FAQ schema on FAQ pages
  if (page.includes('faq') && !hasFAQSchema) {
    issues_local.push({
      issue: `FAQPage schema missing on FAQ page: ${page}`,
      severity: 'WARNING',
    });
  }

  // Check breadcrumbs
  if (!content.includes('breadcrumb') && page.includes('/')) {
    issues_local.push({
      issue: `BreadcrumbList schema missing on nested page: ${page}`,
      severity: 'MINOR',
    });
  }
...
function runAudit() {
  log('\n=== RAINUKA THERAPY: ON-PAGE SEO AUDIT #2 ===\n', 'cyan');
  log('Scanning 30+ pages for: Title tags, Meta descriptions, H1-H3 hierarchy,');
  log('URLs, Internal linking, and Schema markup\n', 'cyan');
  ...
  log(`Found ${pageFiles.length} pages to audit\n`, 'green');

  // Audit each page
  for (const filePath of pageFiles.sort()) {
    ...
    const titleIssues = auditTitleTag(pageName, metadata.title);
    const descIssues = auditMetaDescription(pageName, metadata.description);
    const h1Issues = auditH1Tag(pageName, metadata.h1, metadata.title);
    const hierarchyIssues = auditHeaderHierarchy(pageName, metadata.h1, metadata.h2s, metadata.h3s);
    const urlIssues = auditURLStructure(filePath);
    const linkIssues = auditInternalLinking(content, pageName);
    const schemaIssues = auditSchemaMarkup(content, pageName);

    // Collect all issues for this page
    const allIssues = [...titleIssues, ...descIssues, ...h1Issues, ...hierarchyIssues, ...urlIssues, ...linkIssues, ...schemaIssues];

    // Log page summary
    if (allIssues.length > 0) {
      log(`\n📄 ${pageName}`, 'yellow');
      log(`   Title: ${metadata.title ? metadata.title.substring(0, 60) + '...' : '(missing)'}`);
      log(`   H1: ${metadata.h1 ? metadata.h1.substring(0, 50) + '...' : '(missing)'}`);

      for (const iss of allIssues) {
        const severity = iss.severity === 'CRITICAL' ? '🔴' : iss.severity === 'WARNING' ? '🟡' : '🔵';
        log(`   ${severity} ${iss.severity}: ${iss.issue}`);
        addIssue('ON-PAGE', pageName, iss.issue, iss.severity);
      }
    }
  }
  ...
  // Save report
  const reportPath = path.join(__dirname, 'SEO_AUDIT_ON_PAGE_DETAILED.md');
  fs.writeFileSync(reportPath, report);

  log(`\n✓ Full audit report saved to: SEO_AUDIT_ON_PAGE_DETAILED.md`, 'green');
  log(`Total issues found: ${issueCount}\n`, 'bold');
  ...
  log(`📊 Issue Breakdown:`, 'cyan');
  log(`   🔴 CRITICAL: ${critical}`, 'red');
  log(`   🟡 WARNING: ${warning}`, 'yellow');
  log(`   🔵 MINOR: ${minor}\n`, 'cyan');

  // Top audit categories
  const byCategory = {};
  issues.forEach((i) => {
    byCategory[i.category] = (byCategory[i.category] || 0) + 1;
  });

  log(`📋 Issues by Category:`, 'cyan');
...
function generateAuditReport() {
  let report = `# Wonderloud Therapy: On-Page SEO Audit #2
...
- **CRITICAL:** ${issues.filter((i) => i.severity === 'CRITICAL').length} (blocks indexing/ranking)
- **WARNING:** ${issues.filter((i) => i.severity === 'WARNING').length} (reduces CTR, engagement)
- **MINOR:** ${issues.filter((i) => i.severity === 'MINOR').length} (polish, best practices)
